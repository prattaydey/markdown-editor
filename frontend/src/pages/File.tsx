  ...
  return (
    <main className="w-full bg-zinc-900 text-gray-300">
      ...
      <div className={`w-full transition-all duration-300 ${isSidebar ? 'ml-[250px]' : ''}`}>
        <Navbar isSidebar={isSidebar} toggleSidebar={toggleSidebar} username={username as string} fileId={fileId as string} fileName={fileName} setFileName={setFileName} currentMarkdown={markdown} files={files} setFiles={setFiles} />
      </div>
      ...
      {files.length == 0 ? (
        <div className="w-full h-full flex items-center justify-center">
          <EmptyPage user={user} files={files} setFiles={setFiles}/>
        </div>
      ) : (
